...
public class NewController {
	...
	@RequestMapping("/")
	public String index() { // 기본 요청을 처리하는 메서드 (메뉴)
		return "index";
	}
	
	// showInfo 요청 : 컨트롤러에서 데이터를 model객체에 저장해서 view로 전달하는 메서드
	@RequestMapping("/showInfo")
	public String showInfo(Model model) { // 데이터를 뷰로 전달 : model 객체를 통해서
		model.addAttribute("name", "홍길동");
		model.addAttribute("age", 30);
		return "showInfo"; // 뷰 페이지 이름 - /WEB-INF/views/showInfo.jsp
	}
	...
	public ModelAndView showInfo3(Model model, ModelAndView mv) {
		model.addAttribute("name", "홍길동");
		mv.addObject("name", "이몽룡"); // ModelAndView 우선
		mv.addObject("age", 23);
